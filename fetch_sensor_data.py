...
import time # Pour simuler un délai si besoin

# --- Configuration Constants ---
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
CONFIG_FILE_PATH = os.path.join(SCRIPT_DIR, '..', 'config', 'sensors.json') # Chemin relatif au script
OUTPUT_FILE_PATH = os.path.join(SCRIPT_DIR, '..', 'data.json') # Sortie à la racine du projet
SENSOR_ENDPOINT = "/readings"
SENSOR_PORT = 5000 # Port par défaut (pour le simulateur ou si ESPs configurés sur ce port)
REQUEST_TIMEOUT = 5 # Secondes avant d'abandonner la requête vers un capteur
HISTORY_LIMIT = 144 # Nombre max d'entrées dans l'historique (ex: 144 * 10mn = 24h)
...
def fetch_sensor_data(sensor_id, sensor_config):
    ...
    try:
        ...
        # Si on arrive ici c'est souvent une erreur HTTP (offline simulé)
...
def update_data_file(existing_data, fetch_results, sensor_config_all):
    ...
    # Vérifier les capteurs configurés mais non présents dans les résultats du fetch actuel
    # (devrait moins arriver avec cette structure, mais sécurité)
    # for configured_id in sensor_config_all:
    #    if configured_id not in updated_ids:
             # Gérer ce cas ? Normalement couvert par l'initialisation au dessus.
    #         print(f"[DEBUG] Capteur {configured_id} configuré mais non traité dans ce cycle.")
...
def main():
    ...
    if not sensor_config:
        print("[FIN] Arrêt du script à cause d'une erreur de configuration.")
        return # Arrêter si la config ne peut pas être lue
    ...
    if isinstance(sensor_config, dict):
        for sensor_id, config_details in sensor_config.items():
            ...
            # time.sleep(0.5) # Petite pause entre les requêtes si nécessaire
    else:
        print("[ERREUR] Le format de la configuration n'est pas un dictionnaire attendu.")
        print("[FIN] Arrêt du script.")
        return
